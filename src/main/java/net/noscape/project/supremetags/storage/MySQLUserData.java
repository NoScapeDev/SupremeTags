...
public class MySQLUserData {
    ...
    public void createPlayer(Player player) {
        try {
            if (!exists(player)) {
                PreparedStatement statement = SupremeTags.getMysql().getConnection().prepareStatement(
                        "INSERT INTO `users` " +
                                "(Name, UUID, Active) " +
                                "VALUES " +
                                "(?,?,?)");
    ...
    public static void setActive(OfflinePlayer player, String identifier) {
        try {
            PreparedStatement statement = SupremeTags.getMysql().getConnection().prepareStatement("UPDATE `users` SET Active=? WHERE (UUID=?)");
